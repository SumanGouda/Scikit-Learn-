...
with st.sidebar:
    ...
    selected_yr = st.selectbox(
        "Select Year",
        options = np.arange(2018, 2026).tolist(),
        index=0  # default selection
    )
    from fastf1 import events
    schedule = events.get_event_schedule(selected_yr)
    available_gps = schedule[schedule['EventFormat'] == 'conventional']['EventName'].tolist()
    selected_gp = st.selectbox(
        "Select Grand Prix",
        options = available_gps,
        index=0
    )
    


st.title('🏎️ Get the qualifying lap time of the driver')
